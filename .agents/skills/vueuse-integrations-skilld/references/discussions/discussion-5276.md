...
The error about missing types means you need to run the build step before starting the dev server.
On a fresh clone, the correct sequence is:

1. `pnpm i`
2. `pnpm build:packages`
3. `pnpm dev`  
   This builds all internal packages and generates the required type files. Skipping this step
   causes missing import errors and can break both the dev server and unit tests, so test failures
   are expected until you build first ([source](https://github.com/vueuse/vueuse/issues...
