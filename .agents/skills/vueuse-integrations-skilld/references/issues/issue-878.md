...
Firstly, thanks for maintaining such a great collection of utilities. It's always the first place I
check before implementing a composable myself, and very often, I don't end up needing to because you
guys already have :-)

I'm wondering if it would make sense for `createSharedComposable` to memoize the result based on the
arguments. This definitely makes sense in my use case, and I can't think of a situation where you
wouldn't want this. The following, for example, seems incorrect to me, but that's the way it works
currently:
