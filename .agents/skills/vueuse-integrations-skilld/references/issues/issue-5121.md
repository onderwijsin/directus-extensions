...
After upgrading to VueUse v14, the Nuxt build fails, even when using Nitro 2.18.2, which was
expected to fix the related issue.
...
I faced the same issue. For a workaround, I stopped using the `useWindowSize` composable from the
code and directly used `window.innerWidth`.
