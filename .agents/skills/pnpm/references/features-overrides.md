...
> The `pnpm.overrides` field in `package.json` is **no longer read** (pnpm no longer reads any
> settings from `package.json#pnpm`). Move overrides to `pnpm-workspace.yaml`.
