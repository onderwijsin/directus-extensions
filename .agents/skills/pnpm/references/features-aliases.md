...
Distinct from package aliases: a `namedRegistries` prefix selects _which registry_ a package is
fetched from.
...
3. **Prefer overrides for global replacement**: to replace a package everywhere, use `overrides` (in
   `pnpm-workspace.yaml`) instead of aliases
