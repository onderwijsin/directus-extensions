...
> `patchedDependencies` (like all pnpm settings) now lives in `pnpm-workspace.yaml`, not the
...
A version-less key (`express:`) patches every installed version. All workspace packages using a
matching version get the patch.
...
> `ignorePatchFailures` was **removed** in v11. A patch that fails to apply now always throws. When
> several patches are grouped, all errors are reported together at the end.
