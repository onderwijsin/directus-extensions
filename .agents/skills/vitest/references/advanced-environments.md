...
For real browser testing, use Vitest Browser Mode. In **v4 the provider is an object** (not a
string), and the context imports from `vitest/browser`:
...
> v5: DOM-environment global assignments (e.g. `window.innerWidth`) now propagate to the underlying
> jsdom/happy-dom implementation. Locators are also exact/strict by default.
