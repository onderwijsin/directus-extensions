...
Prefer `expect.soft` for **non-critical assertions** — it marks the test failed but continues so all
...
`expect.assert` throws at runtime **and** narrows the TypeScript type (unlike
`toBeTruthy`/`toBeDefined`, which return `void`):
