...
Suite-level hooks (`beforeAll`/`afterAll`/`aroundAll`) only see **file/worker-scoped** fixtures, and
the **global** `beforeAll`/`afterAll` cannot access custom fixtures — use `test.beforeAll` etc.
