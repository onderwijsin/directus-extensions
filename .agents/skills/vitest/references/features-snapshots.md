...
In CI (`process.env.CI`), Vitest **never writes** snapshots: mismatches, missing snapshots, and
**obsolete snapshots** (entries no longer matching any test) all fail the run.
...
Build matchers on the composable `Snapshots` helpers from `vitest` (replaces importing from
`jest-snapshot`):
