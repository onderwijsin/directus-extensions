...
> v4 simplified default `exclude` to only `node_modules`/`.git`. Prefer `test.dir` to limit where
> tests are found; spread `configDefaults.exclude` to restore the old excludes.
