...
> **v4 pool rework:** `poolOptions` was removed — all pool settings are now top-level.
> `singleThread`/`singleFork` become `maxWorkers: 1, isolate: false`. VM `memoryLimit` is
> `vmMemoryLimit`. These can now be set **per project**.
...
- Files run in parallel by default (`pool: 'forks'`); tests within a file run sequentially unless
  `.concurrent`
- `concurrent` only speeds up tests that **await** (I/O, timers); pure sync tests still block the
  thread
