...
A computed that depends on other reactive state, but only updates after that other state has stopped
changing for a certain time.

Maybe there is some way to use the existing function to achieve this, but I have trouble seeing it.

Note: In my current use-case, I actually only need the computed to fire on the next update, like
watch with the option `{ flush: 'post' }`.
...
Edit: Just found out about `refDebounced` + `toRef` + getter. Not sure how good it is, but it's much
shorter than `watchDebounced` approach

```ts
const preview = refDebounced(
  toRef(() => generatePreview(props.inputText)),
  1000,
)
```
