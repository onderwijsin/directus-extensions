...
When running the newest beta version of `useChangeCase` it fails. It works fine when running in
development(Nuxt), but it fails after build(Nuxt). The demo page for useChangeCase is also broken
when selecting the beta version.

The code in question(`packages/integrations/useChangeCase/index.ts`):

When logging out `changeCase` imported in module it becomes clear that doing `Object.values` won't
work as it would have to check the keys in order to match the `endsWith("Case")`:
