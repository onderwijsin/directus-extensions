...
const obj = ref({a:10,b:0,c:0}) const aTransition = useTransition(ref(obj.value.a), { duration: 500
})

watch( () => obj.value.a, (newVal) => { aTransition.value = newVal } )

const transitionedValues = computed(() => ({ a: aTransition }))

const addProp=()=>{ obj.value.a += 100 obj.value.b += 255 obj.value.c += 140

}

<text>{{transitionedValues.a}},{{obj.a}}</text> <button @click="addProp">addProp</button>
