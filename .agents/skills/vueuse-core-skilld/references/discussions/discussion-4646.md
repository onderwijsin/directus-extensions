...
`useFetch` in vueuse does not natively support streaming responses. However, you can manually
process the response stream by using the `afterFetch` option. This allows you to define a custom
function to handle the response data after the fetch request is completed.
