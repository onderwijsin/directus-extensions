...
number: 5052
title:
  BUG | `useMagicKeys` | Releasing held keys during a drag still show as being held during and after
  a drag
category: Q&A
created: 2025-09-04
...
I have modified the useMagicKeys example from the documentation by adding a div with
`draggable="true"`. See replication link below
...
> From the moment that the user agent is to **initiate the drag-and-drop operation**, until the end
> of the drag-and-drop operation, device input events (e.g. mouse and keyboard events) must be
> suppressed.
