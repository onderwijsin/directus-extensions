...
I created a `fetcher` using `createFetch` to intercept some request errors in the app, but some
special cases i need to override the `afterFetch` option of `fetcher`:
...
const { data } = fethcer('/bar', {
  afterFetch(ctx) {
    // i tried replacing the default `afterFetch` option here, but i didn't
...
Hey @hequ9! I'm here to help you with your code questions and bugs. Let's see if we can solve this
together!

To override the `afterFetch` option so that only the new `afterFetch` function is executed, you can
use the `combination: 'overwrite'` option when creating the fetch instance. Here is an example:
