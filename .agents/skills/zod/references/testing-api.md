...
Always use `safeParse()` in tests. `parse()` throws, which crashes the test instead of producing a
clear failure.
...
it('targets the correct field', () => {
  ...
  expect(result.success).toBe(false)
  if (!result.success) {
    const confirmError = result.error.issues.find((i) => i.path.join('.') === 'confirm')
...
it('validates request body', async () => {
  const res = await request(app).post('/api/users').send({ name: '', email: 'invalid' }).expect(400)
...
it('validates async refinements', async () => {
  const UniqueEmail = z
    .email()
    .refine(async (email) => !(await db.exists(email)), { error: 'Already registered' })
...
Assert that schemas are imported from boundary layers, not from domain code. This enforces the
architectural rule that parsing happens at boundaries.
...
describe('schema architecture', () => {
  it('domain layer does not import raw schemas', () => {
    ...
    const result = execSync('grep -r "from.*schemas" src/domain/ || true', { encoding: 'utf-8' })
...
Export schemas as JSON Schema and commit snapshots. CI fails when schemas change without updating
snapshots.
...
for (const [name, schema] of Object.entries(schemas)) {
  writeFileSync(`snapshots/${name}.json`, JSON.stringify(z.toJSONSchema(schema), null, 2) + '\n')
}
```
...
describe('schema drift detection', () => {
  it('UserSchema matches committed snapshot', () => {
    ...
    const committed = JSON.parse(readFileSync('snapshots/User.json', 'utf-8'))
