...
describe('UserSchema', () => {
  ...
  it('rejects negative age', () => {
    expect(UserSchema.safeParse({ ...validUser, age: -1 }).success).toBe(false)
  })
})
```
...
it('API rejects invalid input with field errors', async () => {
  const res = await request(app).post('/api/users').send({ name: '', email: 'bad' }).expect(400)
...
writeFileSync('snapshots/Order.json', JSON.stringify(jsonSchema, null, 2) + '\n')
