...
Not deprecations — working code that a newer API does better. An anti-pattern is not only a thing
that breaks; it is also a thing you are maintaining that the library will now maintain for you.
...
The transform pair is where serialisation bugs live: someone changes the parse side and the write
side keeps the old shape. A codec makes that impossible to express.
...
Matters against JSON, which cannot express an explicit `undefined`, and against TypeScript's
...
Hand-written JSON Schema alongside a Zod schema is two sources of truth that agree only until
