...
import { EditMultipleParentsChildrenError, EditMultipleParentsError, emitDelete, emitUpdate, findArchiveFieldInCollection, findChildren, findExistingItems, findFieldsInCollection, getPathValue, getSlugValue, preventRecursiveAncestory } from "./helpers";
...
export default defineHook(({ filter }, hookContext) => {
	...
	filter("items.update", async (payload, meta, eventContext) => {
		...
		path = Object.keys(payload).length === 1 && pathField && (payload as Record<string, any>)[pathField.field]
			? {
					key: pathField.field,
					value: (payload as Record<string, any>)[pathField.field]
				}
			: (await getPathValue(payload, meta, { pathField, slug: slug as { key: string; value: string } }, eventContext, hookContext));
