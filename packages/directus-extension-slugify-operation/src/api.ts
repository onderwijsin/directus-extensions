...
import { extensions, locales } from "./constants";

slugify.extend(extensions);

const _localeValues = locales.map((locale) => locale.value);
type Locale = typeof _localeValues[number];
...
export default defineOperationApi<Options>({
	...
	handler: async ({ fields, output_key, locale, make_unique, lowercase }, context) => {
		...
		if (data.$trigger.payload[output_key]) {
			...
			if (values.length > 0 && values.length < fields.length && data.$trigger?.event.includes(".update")) {
