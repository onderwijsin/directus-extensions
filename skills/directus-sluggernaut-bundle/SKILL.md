...
| `SLUGGERNAUT_NORMALIZE_REDIRECTS`                  |     `unset` | `trailing-slash` or `no-trailing-slash`                   | Persistence policy for non-root redirect paths. When unset, the supplied trailing-slash form is preserved. Validation compares paths without trailing slashes regardless of this setting. |
