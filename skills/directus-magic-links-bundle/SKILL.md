...
| `MAGIC_LINKS_EMAIL_SUBJECT`                                    | `Your sign-in link`                                | Optional non-empty email subject override.                                   |
...
gateway as an additional deployment control. When the per-user budget is exhausted, Directus
returns its standard `HitRateLimitError` response with HTTP status `429`; stop retrying the user's
links until the budget expires or use the application's normal sign-in flow.
...
`ip`, `user_agent`, and `preview_text`, alongside Directus project variables. The included template
renders `preview_text` as hidden inbox preheader content. Directus `MailService` supports the
subject line but not a separate preview-text metadata field, so custom templates must render
`{{ preview_text }}` near the start of the HTML body if they need an inbox preview. Configure
`MAGIC_LINKS_EMAIL_SUBJECT` and `MAGIC_LINKS_EMAIL_PREVIEW_TEXT` for locale-specific copy. Configure
